...
export const TransactionBreakdown: React.FC<TransactionBreakdownProps> = ({
  ...
  const totalSales = sales.reduce(
    (sum, t) => sum + parseFloat(t._sum.amount),
    0,
  );
  ...
  const totalPlatformFees = platformFees.reduce(
    (sum, t) => sum + parseFloat(t._sum.amount),
    0,
  );
          ...
          <View style={styles.headerText}>
            ...
            <Text
              style={[
                styles.cardAmount,
                {
                  color: isPositive(amount.toString()) ? '#10B981' : '#0F172A',
                },
              ]}
            >
              {amount >= 0 ? '+' : '-'}
